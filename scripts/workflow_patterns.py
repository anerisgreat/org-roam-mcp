...
"""
    ...
    async def auto_assign_agents(self, task: WorkflowTask) -> Dict[str, str]:
        ...
        if any(keyword in task_lower for keyword in ["research", "paper", "study", "analysis"]):
            assignments[
                "research_agent"
            ] = "Conduct comprehensive research using Zotero and Google Scholar integration"

        if any(keyword in task_lower for keyword in ["implement", "code", "develop", "build"]):
            assignments[
                "development_agent"
            ] = "Implement the solution following best practices and architectural guidelines"

        if any(keyword in task_lower for keyword in ["test", "verify", "validate", "automation"]):
            assignments[
                "testing_agent"
            ] = "Create and execute comprehensive tests using Playwright automation"

        if any(keyword in task_lower for keyword in ["deploy", "integrate", "setup", "configure"]):
            assignments[
                "integration_agent"
            ] = "Handle deployment and system integration requirements"
...
RESEARCH_WORKFLOW = {
    ...
    "tasks": [
        ...
        },
        {
            ...
            "agents": {"development_agent": "Code the solution with comprehensive documentation"},
